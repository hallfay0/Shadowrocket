...
> > 
> > 本设置的优先级高于软件设置内的相关选项。当参数的值设定为 `3` 时的效果等同于启用 [Tun模式](#代理类型)：[设置](#设置页面) > 代理 > [代理类型](#代理类型) > None。当参数的值设定为 `5` 时的效果等同于启用 [兼容模式](#兼容模式)：[设置](#设置页面) > 代理 > [兼容模式](#兼容模式)。其他数值或为兼容其他软件所有，其有效性需咨询作者
>
> **`always-ip-address`**
>
